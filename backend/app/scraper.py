...
async def download_landing_page_assets(url: str, output_dir: Optional[str] = None) -> dict:
    ...
    async with httpx.AsyncClient(follow_redirects=True, timeout=30.0) as client:
        ...
        async def _dl(target_type, asset_url):
            if asset_url in seen: return
            seen.add(asset_url)
            try:
                fname = os.path.basename(urlparse(asset_url).path)
                if not fname:
                    fname = f"asset_{hashlib.md5(asset_url.encode()).hexdigest()[:8]}"
        ...
        return {
            "url": url,
            "summary": {t: len([d for d in downloaded if d["type"] == t]) for t in ["images", "css", "js", "fonts", "other"]},
